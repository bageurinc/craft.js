...
const EventHandlerContext = React.createContext(null);
const useEventHandler = () => React.useContext(EventHandlerContext);
...
function Element$1({ id, children, ...elementProps }) {
  ...
  const [linkedNodeId] = React.useState(() => {
...
const Frame = ({ children, json, data }) => {
  ...
  if (!isLoaded.current) {
    ...
    if (initialData) {
      ...
      const rootNode = React__default['default'].Children.only(children);
...
const Methods = (state, query) => {
  ...
  const addNodeTreeToParent = (tree, parentId, addNodeType) => {
    ...
    if (!parentId && tree.rootNodeId === utils.ROOT_NODE) {
      return;
    }
...
const resolveComponent = (resolver, comp) => {
  ...
  invariant__default['default'](
    resolvedName,
    utils.ERROR_NOT_IN_RESOLVER.replace('%node_type%', getComponentName(comp))
  );
  return resolvedName;
};
...
function createNode(newNode, normalize) {
  ...
  let id = newNode.id || utils.getRandomId();
  const node = {
    id,
...
class CoreEventHandlers extends utils.EventHandlers {
  handlers() {
    return {
      connect: (el, id) => {},
      select: (el, id) => {},
...
class DerivedCoreEventHandlers extends utils.DerivedEventHandlers {}
...
class Positioner {
  ...
  onScroll(e) {
    ...
    const rootNode = this.store.query.node(utils.ROOT_NODE).get();
...
const Editor = ({ children, ...options }) => {
  ...
  return React__namespace.createElement(
    ...
    { value: context },
    React__namespace.createElement(Events, null, children)
  );
};
...
const getTestNode = (parentNode) => {
  ...
  const validParentNode = createNode(cloneDeep__default['default'](parentNode));
