...
const StyledDiv = styled.div `
  ...
  align-items: center;
  padding: 4px 10px;
  background: ${(props) => props.$selected ? props.$theme.bgSelected : 'transparent'};
  color: ${(props) => props.$selected ? props.$theme.textSelected : props.$theme.textPrimary};
  svg {
    fill: ${(props) => props.$selected ? props.$theme.iconSelected : props.$theme.iconPrimary};
    margin-top: 2px;
  }
  .inner {
    flex: 1;
...
const Hide = styled.a `
  ...
  &:after {
    ...
    left: 2px;
    top: 3px;
    background: ${(props) => props.$selected ? props.$theme.iconSelected : props.$theme.iconPrimary};
...
export const DefaultLayerHeader = () => {
    ...
    return (React.createElement(StyledDiv, { "$selected": selected, ref: (dom) => {
            drag(dom);
        }, "$depth": depth, "$theme": theme },
        React.createElement(Hide, { "$selected": selected, "$isHidden": hidden, "$theme": theme, onClick: () => actions.setHidden(id, !hidden) },
            React.createElement(Eye, null)),
        React.createElement("div", { className: "inner" },
            React.createElement("div", { ref: (dom) => {
                    layerHeader(dom);
                } },
